...
function onMobDeath(mob,player,isKiller)
    ...
    if stage == 4 then
        ...
        if dayEntered >= 4 and dayEntered <= 5 then
            printf("Debug]: Success! Day check passed Ice/Thunder | Setting pass to 1")
			player:setLocalVar("Salvage-ZhayolmFL4-Pass", 1)
        else
            printf("[Debug]: Failed day check: Ice/Thunder | Setting pass to 10")
