...
function onMobDeath(mob,player,isKiller)
    ...
    if stage == 4 then
        ...
        if dayEntered >= 2 and dayEntered <= 3 then
            printf("Debug]: Success! Day check passed Wind/Water | Setting pass to 1")
			player:setLocalVar("Salvage-ZhayolmFL4-Pass", 1)
        else
            printf("[Debug]: Failed day check: Wind/Water | Setting pass to 10")
