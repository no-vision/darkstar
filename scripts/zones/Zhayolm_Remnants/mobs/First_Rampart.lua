...
function onMobDeath(mob,player,isKiller)
	...
	if stage == 4 then
		...
		if dayEntered >= 0 and dayEntered <= 1 then
			printf("Debug]: Success! Day check passed Fire/Earth | Setting pass to 1")
			player:setLocalVar("Salvage-ZhayolmFL4-Pass", 1)
		else
			printf("[Debug]: Failed day check: Fire/Earth | Setting pass to 10")
