...
function onUseAbility(pet, target, skill, action)
    ...
    local dmgmod = MobBreathMove(pet, target, 0.185, pet:getMainLvl()*15, dsp.magic.ele.LIGHTNING) -- Works out to (hp/6) + 15, as desired
...
<<<<<<< HEAD
    ...
    target:delHP(dmg)
=======
    local dmg = AbilityFinalAdjustments(dmgmod,pet,skill,target,dsp.attackType.BREATH,dsp.damageType.LIGHTNING,MOBPARAM_IGNORE_SHADOWS)
